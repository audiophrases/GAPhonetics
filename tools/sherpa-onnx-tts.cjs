...
// Copied from OpenClaw skill wrapper to avoid Node ESM resolution issues.
// Requires env:
...
if (!runtimeDir || !modelDir) usage("Missing runtime/model directory.");
...
if (!fs.existsSync(bin)) usage(`TTS binary not found: ${bin}`);
...
const child = spawnSync(
  bin,
  [`--vits-model=${modelFile}`, `--vits-tokens=${tokensFile}`, `--vits-data-dir=${dataDir}`, `--output-filename=${outputPath}`, text],
  { stdio: "inherit", env },
);

if (typeof child.status === "number") process.exit(child.status);
if (child.error) console.error(child.error.message || String(child.error));
process.exit(1);
